...
@WebServlet(name = "CheckOption1", value = "/CheckOption1")
public class CheckOption1 extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String option = Fomat.getStringOption(bill.getFullName(), bill.getPhone(), bill.getAddress(), bill.getPrice(), "" + bill.getDayBooking(), "Đơn hàng của bạn sẽ được thu hồi và hoàn tiền lại! ");
        try {
            Email.sendMailHTML(user.getEmail(), " Hoàn tiền đơn hàng! ", option );
            BillService.deleteBill(idBill);
            response.setContentType("text/plain");
            response.getWriter().write(String.valueOf("Hoàn tiền thành công."));
        } catch (Exception e) {
